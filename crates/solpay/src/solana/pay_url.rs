...
pub struct TransferRequest<'a> {
    ...
    pub amount_ui: &'a str,
    pub spl_token: &'a Pubkey,
    pub reference: &'a Pubkey,
    pub label: &'a str,
    pub message: Option<&'a str>,
}

/// Build the Solana Pay URL for a USDC (SPL token) transfer request.
pub fn build_transfer_request_url(req: &TransferRequest) -> String {
    let label_enc = utf8_percent_encode(req.label, NON_ALPHANUMERIC);
    let mut url = format!(
        "solana:{recipient}?amount={amount}&spl-token={mint}&reference={reference}&label={label}",
